...
CDigitalBeamFormer::DBFprocessorCP(EMSTIME tm )
{
	...
	CToaFoaProcessor toaFoaProc(m_qrefDBFBeamVectors, m_ulSatellites, m_aPassSchedule); 
	...
	if (m_asRawTimeSeries )
	{
		...
		hr = toaFoaProc.InitializeTOAFOA( tm );
...
CDigitalBeamFormer::DBFprocessorEP( EMSTIME tm )
{
	...
	if ( m_asRawTimeSeries )
	{
		...
		hr = ApplyDBFBeamVectors( m_ulSatellites, bTimeFreqFlag, bBandwidthFlag, toaFoaProc );
		...
		toaFoaProc.IdentifyTOAFOA( tm, m_acDBFBeamVectors );
...
EMS_RESULT
CDigitalBeamFormer::ApplyDBFBeamVectors( ULONG m_ulSatellites, bool bTimeFreqFlag, bool bBandwidthFlag, CToaFoaProcessor toaFoaProc )
{
	EMS_RESULT hr = EMS_FALSE;
	...
	if ( m_acDBFBeamVectors && m_acMatrix )
	{
		...
		for ( int isat = 0; isat < m_ulSatellites; isat++ )
		{
			...
			toaFoaProc.SatelliteTOAFOA( isat );
