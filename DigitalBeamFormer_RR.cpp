...
CDigitalBeamFormer::Initialize( const TCHAR *cDir )
{
	...
	m_acTemp1	= new EMSCOMPLEX[DBF_LOG19_SIZE+1];
	m_acTemp2	= new EMSCOMPLEX[DBF_LOG19_SIZE+1];
...
CDigitalBeamFormer::_OutputMaxPowerPhases( int numSats )
{
	...
	double dPeakPhase[DBF_NUM_ELEMENTS];
	double dStdDev = 0.0;
	double dPowerThreshold = 15.00;
	double dNoise = 0.0;
	double dMean = 0.0;
	const ULONG DETECTIONOFFSET = 100000; // 50 kHz @ 500k sample rate
	const ULONG DETECTIONBANDWIDTH = 200000; // 100 kHz bandwidth

	if( numSats > DBF_MAX_SATELLITES )
		numSats = DBF_MAX_SATELLITES;
	for( int i = 0; i < numSats; i++ )
	{
		emscbPowerSpectr(m_acFFTdata[i], m_afPowerSpectrum, DBF_LOG19_SIZE+1);
		dStdDev = _EMSsMeanStdDev( &m_afPowerSpectrum[DETECTIONOFFSET], DETECTIONBANDWIDTH, &dMean );
		for( ULONG k = 0; k < DBF_LOG18_SIZE; k++ )
		{
	...
	{
		memset( &dPeakPhase[0], 0, DBF_NUM_ELEMENTS * sizeof(double) );
		...
		{
			m_emsDBFBufferPhase.fPhase[icell] = atan2((double )m_aPassSchedule.rec[iSatMax].nPhaseImag[icell],(double) m_aPassSchedule.rec[iSatMax].nPhaseReal[icell] );
			m_emsDBFBufferPhase.fBias[icell] = dPeakPhase[icell] - dPeakPhase[iCentreCell]; //- m_emsDBFBufferPhase.fPhase[ichan];
		}
...
CDigitalBeamFormer::DBFprocessorEP(EMSTIME tm)
{
	...
	if ( m_lpTraceFile )
	{
		fprintf( m_lpTraceFile, "3,%d,%f,%f,%f,%f,%f,%ld",
			m_nCounter, m_dElapsedTime, m_dAverageTime,
			m_dMeanADC, m_dStdDevADC, m_dMaxPower, m_ulStartPPS );

		for (int i = 0; i < m_ulChannels; i++)
		{
			fprintf(m_lpTraceFile,",%02d,%f,%f", i, m_afMean[i], m_afStdDev[i] );
		}
...
CDigitalBeamFormer::ApplyDBFBeamVectors( ULONG m_ulSatellites )
{
	...
	if ( m_acDBFBeamVectors && m_acMatrix && m_acFFTdata)
	{
		...
		for ( int isat = 0; isat < m_ulSatellites; isat++ )
		{
			...
			emscFft( &m_acTemp3[0], DBF_LOG20, EMS_SPL_INV );
			...
			{
				m_afTemp1[i] = m_acTemp3[i].re;
				m_afTemp2[i] = m_acTemp3[i].im;
			}
...
CDigitalBeamFormer::ComputeChannelData( )
{
	...
	for ( icell = 0; icell < DBF_NUM_ELEMENTS; icell++ )
	{
		...
		ULONG ulDownConvertSize = DBF_LOG19_SIZE - ulBand50;
		memcpy(&m_acTemp2[0], &m_acTemp1[ulBand50], (ulDownConvertSize) * sizeof(EMSCOMPLEX)); 
