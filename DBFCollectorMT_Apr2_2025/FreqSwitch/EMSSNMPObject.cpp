...
EMSSNMPObject::EMSSNMPObject(const EMSSNMPObject & other) {
	memset( &_oid, 0, sizeof( _oid ) );
    *this = other;
}

EMSSNMPObject & EMSSNMPObject::operator=(const EMSSNMPObject & other) {
...
EMSSNMPObject::EMSSNMPObject(std::string oid) {
	_bInit = false;
	_binary = false;
	_int = 0;
	_uint = 0;
...
void EMSSNMPObject::setOID(std::string oid) {	
	if(_bInit) {
...
int EMSSNMPObject::compareOID(EMSSNMPObject * pOther) const
{
...
int EMSSNMPObject::compareOID(std::string oid) const
{
	if(!_bInit) {
...
void EMSSNMPObject::setOctets(unsigned char * pValue, unsigned int nValue) {
	if(_octets) {
		free(_octets);
	}
...
std::string EMSSNMPObject::getValueAsString() const {
	std::stringstream ss;
	switch(_syntax) {		
...
void EMSSNMPObject::setValueAsString(std::string value) {	
	switch(_syntax) {		
		case SNMP_SYNTAX_INT:
...
void EMSSNMPObject::setValueAsOctets(char * value, int size)
{
	switch(_syntax) {	
		case SNMP_SYNTAX_OCTETS:
		case SNMP_SYNTAX_OPAQUE:
...
char * EMSSNMPObject::getValueAsOctets(int * size) const {
	char * pChar = NULL;
	int sz = 0;
	switch(_syntax) {	
		case SNMP_SYNTAX_OCTETS:
...
void EMSSNMPObject::setValueAsInteger(__int64 value) {
	switch(_syntax) {		
		case SNMP_SYNTAX_INT:
			setInt(value);
			break;
...
__int64 EMSSNMPObject::getValueAsInteger() const {
	switch(_syntax) {		
		case SNMP_SYNTAX_INT:
...
int EMSSNMPObject::compareValue(EMSSNMPObject * pOther) const
{
	switch(_syntax) {		
		case SNMP_SYNTAX_OCTETS:
		case SNMP_SYNTAX_OPAQUE:
...
std::string EMSSNMPObject::getDisplayInformation() const {
	std::stringstream ss;
	ss << getOID() << " = " << getValueAsString() << " [" << getSyntaxAsString() + "]";
	return ss.str();
}

smiOID * EMSSNMPObject::getBinaryOID() {
	if(!_bInit) {
		return NULL;
	}
	return &_oid;
...
std::string	EMSSNMPObject::getOID() const {
	if(!_bInit) {
		return "";
	}
...
void EMSSNMPObject::setSyntax(int syntax) {
	switch(syntax) {
		case SNMP_INT:
...
int EMSSNMPObject::getSyntax() const {
	switch(_syntax) {		
		case SNMP_SYNTAX_INT:
			return SNMP_INT;
		case SNMP_SYNTAX_OCTETS:
...
std::string EMSSNMPObject::getSyntaxAsString() const {
...
void EMSSNMPObject::setValue(smiVALUE * smiVal)
{
...
void EMSSNMPObject::getValue(smiVALUE * smiVal)
{
