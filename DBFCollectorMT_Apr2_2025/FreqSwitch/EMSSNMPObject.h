...
public:
	EMSSNMPObject(std::string oid = "");
	EMSSNMPObject(const EMSSNMPObject & other);
	virtual ~EMSSNMPObject();
	EMSSNMPObject & operator=(const EMSSNMPObject & other);
	
	// OID
	std::string	getOID()const;
	void setOID(std::string oid);		
	int compareOID(EMSSNMPObject * pOther) const;
	...
	int compareValue(EMSSNMPObject * pOther) const;
