...
EMS_RESULT CDBFPointingAngles::Initialize()
{
	...
	memset( m_asPhaseVector, 0, DBF_NUM_ELEMENTS * DBF_MAX_SATELLITES * 2 * sizeof(short));
												  ...
												  double *dCellPositionY )
{
...

	{
		...
		double dCellPositionX[DBF_NUM_ELEMENTS];
		double dCellPositionY[DBF_NUM_ELEMENTS];

		double R = 0.12; // cell separation in metres

		double dx = (sqrt(3.00)*R) / (2*dWaveLength);
		double dy = R / (2*dWaveLength);

		memcpy( &dCellPositionX[0], 0, DBF_NUM_ELEMENTS * sizeof(double) );
		memcpy( &dCellPositionY[0], 0, DBF_NUM_ELEMENTS * sizeof(double) );
		...
		for ( int i = 0; i < DBF_NUM_ELEMENTS; i = +2 )
		{
