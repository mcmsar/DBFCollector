	...
	//snl added nullptr
{
	...
	m_afTemp2 = NULL;
	m_afChan0 = NULL;
	m_afChanTemp1 = nullptr;
	m_afChanTemp2 = nullptr;
	m_acChanTemp1 = nullptr;
	m_acChanTemp2 = nullptr;
	m_iBeamIDs = NULL;
...
m_txQueue(), m_txQueue2(), m_txThread(), m_txThread2()
{
	m_iObjectID = GetNextObjID();
}
...
CDigitalBeamFormer::~CDigitalBeamFormer( void )
{
	...
	if (m_afChanTemp1) { delete[] m_afChanTemp1; m_afChanTemp1 = nullptr; }
	if (m_afChanTemp2) { delete[] m_afChanTemp2; m_afChanTemp2 = nullptr; }
	if (m_acChanTemp1) { delete[] m_acChanTemp1; m_acChanTemp1 = nullptr; }
	if (m_acChanTemp2) { delete[] m_acChanTemp2; m_acChanTemp2 = nullptr; }

	// Stop async transmit threads before releasing COM objects
	m_txQueue.Stop();
	m_txQueue2.Stop();
	if (m_txThread.joinable())  m_txThread.join();
	if (m_txThread2.joinable()) m_txThread2.join();
	...
	if (m_acAvgCovariance)
	{
		delete[] m_acAvgCovariance;
		m_acAvgCovariance = NULL;
	}
...
CDigitalBeamFormer::Initialize( const TCHAR* cDir )
{
	...
	m_afChanTemp1 = new float[DBF_NUM_ELEMENTS][DBF_LOG20_SIZE];
	m_afChanTemp2 = new float[DBF_NUM_ELEMENTS][DBF_LOG20_SIZE];
	m_acChanTemp1 = new EMSCOMPLEX[DBF_NUM_ELEMENTS][DBF_LOG20_SIZE];
	m_acChanTemp2 = new EMSCOMPLEX[DBF_NUM_ELEMENTS][DBF_LOG19_SIZE];
	memset( m_afChanTemp1, 0, DBF_NUM_ELEMENTS * DBF_LOG20_SIZE * sizeof( float ) );
	memset( m_afChanTemp2, 0, DBF_NUM_ELEMENTS * DBF_LOG20_SIZE * sizeof( float ) );
	memset( m_acChanTemp1, 0, DBF_NUM_ELEMENTS * DBF_LOG20_SIZE * sizeof( EMSCOMPLEX ) );
	memset( m_acChanTemp2, 0, DBF_NUM_ELEMENTS * DBF_LOG19_SIZE * sizeof( EMSCOMPLEX ) );

	m_acAvgCovariance = new EMSCOMPLEX[COVARIANCE_SIZE];
	memset( m_acAvgCovariance, 0, COVARIANCE_SIZE * sizeof( EMSCOMPLEX ) );
	m_fCovAvgAlpha = 0.2f;   // keep 80% history, blend in 20% new each block
	...
	// Start async transmit threads
	if (!m_txThread.joinable())
	{
		m_txQueue.Restart();
		m_txThread = std::thread( [this] {
			_TransmitThreadProc( m_txQueue, m_wsSPIP, CLSID_DataXmitter, IID_IEMSDataTransmitter );
			} );
	}
	if (!m_txThread2.joinable() && !m_wsSPIP2.empty())
	{
		m_txQueue2.Restart();
		m_txThread2 = std::thread( [this] {
			_TransmitThreadProc( m_txQueue2, m_wsSPIP2, CLSID_DataXmitter2, IID_IEMSDataTransmitter );
			} );
	}
...
CDigitalBeamFormer::DBFprocessorEP( EMSTIME tm )
{
	...
	if (m_asRawTimeSeries)
	{
		...
		{
			m_acAvgCovariance[i].re = m_fCovAvgAlpha * m_acCovariance[i].re
				+ (1.0f - m_fCovAvgAlpha) * m_acAvgCovariance[i].re;
			m_acAvgCovariance[i].im = m_fCovAvgAlpha * m_acCovariance[i].im
				+ (1.0f - m_fCovAvgAlpha) * m_acAvgCovariance[i].im;
		}

		//hr = ComputeDBFBeamVectors( m_ulSatellites, m_acCovariance);
		hr = ComputeDBFBeamVectorsMVDR( m_ulSatellites, m_acCovariance );
...
void
CDigitalBeamFormer::_TransmitThreadProc(
	CTransmitQueue& queue, const std::wstring& wsIP,
	REFCLSID clsid, REFIID iid )
{
	CoInitializeEx( NULL, COINIT_MULTITHREADED );
	IEMSDataTransmitter* pDT = nullptr;
	HRESULT hr = CoCreateInstance( clsid, NULL, CLSCTX_ALL, iid, (void**)&pDT );
	if (SUCCEEDED( hr ) && pDT)
	{
		hr = pDT->Connect( wsIP.c_str() );
		if (FAILED( hr )) { printf( "[TransmitThread] Connect failed hr=0x%08X\n", hr ); pDT->Release(); pDT = nullptr; }
	}
	else printf( "[TransmitThread] CoCreateInstance failed hr=0x%08X\n", hr );
	TransmitPacket pkt;
	while (queue.Pop( pkt ))
	{
		if (pDT && pkt.pData && pkt.dwBytes > 0)
		{
			int iSent = 0;
			hr = pDT->Send( pkt.dwBytes, pkt.pData, &iSent );
			if (FAILED( hr )) printf( "[TransmitThread] Send failed hr=0x%08X\n", hr );
		}
	}
	if (pDT) { pDT->Disconnect(); pDT->Release(); }
	CoUninitialize();
}
	...
	for (int iSat = 0; iSat < (int)culNumSats; iSat++)
	{
		...
		// Primary output → m_pDataTransmit (200kHz 1s buffer -> SP) (file fallback on failure)
		{
			...
			if (dwBytes > 0)
			{
				...
				//					lpWaveFile = NULL;
				//				}
				m_txQueue.Push( { abyData, dwBytes } );
				abyData = nullptr;
			}
			//			delete[] abyData;
			if (abyData) { delete[] abyData; abyData = nullptr; }
		}
...
//		if (m_pDataTransmit2)
		if (!m_wsSPIP2.empty())
		{
			...
			ULONG ulOffset = DBF_LOG19_SIZE / 4;
			...
			memcpy( &m_acTemp2[0], &m_acTemp1[ulOffset], ulOffset * 2 * sizeof( EMSCOMPLEX ) );
			...
			if (dwBytes2 > 0)
			{
				...
				m_txQueue2.Push( { abyData2, dwBytes2 } );
				abyData2 = nullptr;
			}
			//			delete[] abyData2;
			if (abyData2) { delete[] abyData2; abyData2 = nullptr; }
			delete[] newBuffer;
		}
	}
...
CDigitalBeamFormer::ProcessAll()
{
	...
	{
		static DWORD s_dwLastNoSatLog = 0;
		DWORD dwNow = GetTickCount();
		if (dwNow - s_dwLastNoSatLog > 10000)
		{
			s_dwLastNoSatLog = dwNow;
			printf( "[ProcessAll] No satellites in pass schedule (ulSatID=%lu, m_ulSatellites=%lu)\n",
				m_aPassSchedule.rec[0].ulSatID, m_ulSatellites );
		}
		return;
	}
...
CDigitalBeamFormer::ComputeDBFBeamVectors( INT nNumBeams, const EMSCOMPLEX* m_acCovariance )
{
	...
	if (EMS_OK == hr)
	{
		...
		if (EMS_OK == hr)
		{
			EMSCOMPLEX acLargestEigenVectors[DBF_MAX_SATELLITES * NUM_CHANNELS];
			memset( acLargestEigenVectors, 0, DBF_MAX_SATELLITES * NUM_CHANNELS * sizeof( EMSCOMPLEX ) );
			// Extracting the nNumBeams largest eigenvectors
			for (int iBeam = 0; iBeam < nNumBeams; iBeam++)
			{
				memcpy( &acLargestEigenVectors[iBeam * NUM_CHANNELS],
					&acNormalizedEigenVectors[(30 - iBeam) * NUM_CHANNELS],
					NUM_CHANNELS * sizeof( EMSCOMPLEX ) );
			}
			...
			for (int ibeam = 0; ibeam < nNumBeams; ibeam++)
			{
				for (int ielement = 0; ielement < DBF_NUM_ELEMENTS; ielement++)
				{
					...
					dPhase = atan2f( fBeamReal, fBeamImag ) * (float)(360.0 / c_dTwoPI);

					m_aTOAFOA.fEigenvector[ibeam * DBF_NUM_ELEMENTS + ielement] = dPhase;
				}
			}
...
//////////////////////////////////////////////////////////////////////////////////////
// MVDR (Capon) Beamformer variant of ComputeDBFBeamVectors.
//
// Instead of deriving beam directions from eigenstructure, each beam weight vector
// is computed directly from the known satellite steering vector (from the pass
// schedule phase data) and the inverted covariance matrix:
//
//   w_i = R^-1 * a_i / (a_i^H * R^-1 * a_i)
//
// This allows closely-spaced satellites to be resolved independently because each
// beam is formed toward a known direction rather than relying on eigenvalue
// separation.  Diagonal loading stabilises the matrix inverse when satellites are
// geometrically close and the covariance is near-singular.
//
// Improvements #1 (MVDR) and #2 (diagonal loading) from design document.
EMS_RESULT
CDigitalBeamFormer::ComputeDBFBeamVectorsMVDR( INT nNumBeams, const EMSCOMPLEX* acCovariance )
{
	if (!acCovariance || nNumBeams < 1 || nNumBeams > MAX_BEAMS)
		return EMS_BAD_PARAM;

	// Eigendecomposition retained for diagnostic eigenvalue logging only.
	// The resulting eigenvectors are not used for beam computation here.
	float        EigenValues[DBF_NUM_ELEMENTS];
	MKL_Complex8 acEigenVectors[DBF_NUM_ELEMENTS * DBF_NUM_ELEMENTS];
	memset( EigenValues, 0, sizeof( EigenValues ) );
	memset( acEigenVectors, 0, sizeof( acEigenVectors ) );
	if (EMS_OK == PerformEigenDecomposition( acCovariance, EigenValues, acEigenVectors ))
		memcpy( &m_aTOAFOA.fEigenvalue[0], EigenValues, DBF_NUM_ELEMENTS * sizeof( float ) );

	// Copy covariance to a mutable working buffer for in-place LU factorization.
	// EMSCOMPLEX (Ipp32fc: {float re, im}) and lapack_complex_float (MKL_Complex8:
	// {float real, imag}) share identical binary layout — the cast below is safe.
	EMSCOMPLEXD acRinv[NUM_CHANNELS * NUM_CHANNELS];
	memcpy( acRinv, acCovariance, NUM_CHANNELS * NUM_CHANNELS * sizeof( EMSCOMPLEXD ) );

	// --- Improvement #2: Diagonal loading ---
	// Adds alpha * (trace(R)/N) * I to regularise the covariance before inversion.
	// This prevents near-singular behaviour when two satellites have similar steering
	// vectors (small angular separation).  Alpha = 0.01 is a conservative starting
	// point; increase toward 0.1 for severely ill-conditioned geometries.
	const float fDiagLoadAlpha = 0.01f;
	float fTrace = 0.0f;
	for (int i = 0; i < NUM_CHANNELS; i++)
		fTrace += acRinv[i * NUM_CHANNELS + i].re;
	float fLoadFactor = (fDiagLoadAlpha * fTrace) / (float)NUM_CHANNELS;
	for (int i = 0; i < NUM_CHANNELS; i++)
		acRinv[i * NUM_CHANNELS + i].re += fLoadFactor;

	// LU factorization then in-place inversion of the loaded covariance matrix.
	// LAPACK_ROW_MAJOR matches the C row-major storage used throughout this module.
	lapack_int ipiv[NUM_CHANNELS];
	lapack_int info = LAPACKE_cgetrf( LAPACK_ROW_MAJOR, NUM_CHANNELS, NUM_CHANNELS,
		(lapack_complex_float*)acRinv, NUM_CHANNELS, ipiv );
	if (info != 0)
	{
		printf( "ComputeDBFBeamVectorsMVDR: cgetrf failed (info=%d)\n", info );
		return EMS_FALSE;
	}
	info = LAPACKE_cgetri( LAPACK_ROW_MAJOR, NUM_CHANNELS,
		(lapack_complex_float*)acRinv, NUM_CHANNELS, ipiv );
	if (info != 0)
	{
		printf( "ComputeDBFBeamVectorsMVDR: cgetri failed (info=%d)\n", info );
		return EMS_FALSE;
	}

	// --- Improvement #1: MVDR weight vector per satellite ---
	// w_i = R^-1 * a_i / (a_i^H * R^-1 * a_i)
	for (int iBeam = 0; iBeam < nNumBeams; iBeam++)
	{
		// Build steering vector from predicted per-channel phase (radians).
		// fPhase[0] is unused; channels are indexed fPhase[1..NUM_CHANNELS].
		EMSCOMPLEXD acSteer[NUM_CHANNELS];
		for (int k = 0; k < NUM_CHANNELS; k++)
		{
			float fPh = m_aPassSchedule.rec[iBeam].fPhase[k + 1];
			acSteer[k].re = cosf( fPh );
			acSteer[k].im = sinf( fPh );
		}

		// Rinv_a = R^-1 * a_i  (31x31 matrix times 31x1 steering vector)
		EMSCOMPLEXD acRinvA[NUM_CHANNELS];
		memset( acRinvA, 0, sizeof( acRinvA ) );
		emscbMatrixMultiplyd( acRinv, NUM_CHANNELS, NUM_CHANNELS,
			acSteer, NUM_CHANNELS, 1,
			acRinvA );

		// Denominator: a_i^H * Rinv_a  (Hermitian inner product).
		// For a Hermitian positive-definite Rinv this is real and positive;
		// the imaginary part should be near zero after diagonal loading.
		double dDenomRe = 0.0, dDenomIm = 0.0;
		for (int k = 0; k < NUM_CHANNELS; k++)
		{
			dDenomRe += (double)acSteer[k].re * acRinvA[k].re
				+ (double)acSteer[k].im * acRinvA[k].im;
			dDenomIm += (double)acSteer[k].re * acRinvA[k].im
				- (double)acSteer[k].im * acRinvA[k].re;
		}

		double dDenomMagSq = dDenomRe * dDenomRe + dDenomIm * dDenomIm;
		if (dDenomMagSq < 1e-20)
		{
			printf( "ComputeDBFBeamVectorsMVDR: near-zero denominator for beam %d, skipping\n", iBeam );
			continue;
		}

		// w_i = Rinv_a / denom  (element-wise complex division by scalar denom)
		for (int k = 0; k < NUM_CHANNELS; k++)
		{
			double re = acRinvA[k].re;
			double im = acRinvA[k].im;
			m_acDBFBeamVectors[iBeam * NUM_CHANNELS + k].re =
				(float)((re * dDenomRe + im * dDenomIm) / dDenomMagSq);
			m_acDBFBeamVectors[iBeam * NUM_CHANNELS + k].im =
				(float)((im * dDenomRe - re * dDenomIm) / dDenomMagSq);
		}
	}

	// Store beam vector phases in m_aTOAFOA for downstream compatibility
	// (same convention as ComputeDBFBeamVectors)
	float fBeamReal, fBeamImag;
	float dPhase;
	for (int ibeam = 0; ibeam < nNumBeams; ibeam++)
	{
		for (int ielement = 0; ielement < DBF_NUM_ELEMENTS; ielement++)
...
CDigitalBeamFormer::ApplyDBFBeamVectors( ULONG m_ulSatellites, bool bTimeFreqFlag, bool bBandwidthFlag )
{
	...
	if (m_acDBFBeamVectors && m_acMatrix)
	{
		...
		for (int isat = 0; isat < m_ulSatellites; isat++)
		{
			...
			{
				// Fused multiply-accumulate: m_acTemp1 += weight * row[ielem]
				// Replaces memcpy + emscbMpy1 + emscbAdd2 with a single BLAS caxpy pass.
				cblas_caxpy( (MKL_INT)DBF_LOG19_SIZE,
					(const MKL_Complex8*)&m_acDBFBeamVectors[isat * DBF_NUM_ELEMENTS + ielem],
					(const MKL_Complex8*)&m_acMatrix[ielem * DBF_LOG19_SIZE], 1,
					(MKL_Complex8*)m_acTemp1, 1 );
			}
			...
			if (bBandwidthFlag)
			{
				...
				ulOutputSize /= 2;
				for (ULONG i = 0; i + 4 < DBF_LOG20_SIZE; i += 5)
				{
...
CDigitalBeamFormer::ComputeCovariance( ULONG ulNumSamples )
{
	...
	// Compute sample covariance R[i,j] = sum_l conj(X[i,l]) * X[j,l] using BLAS Level 3.
	// cblas_cherk(NoTrans) fills the upper triangle with C[i,j] = sum_l X[i,l]*conj(X[j,l]),
	// which is conj(R[i,j]).  A fixup pass then conjugates the upper triangle and fills
	// the lower as the Hermitian conjugate, restoring the original convention.
	cblas_cherk( CblasRowMajor, CblasUpper, CblasNoTrans,
		(MKL_INT)NUM_CHANNELS, (MKL_INT)ulLength,
		1.0f,
		(const MKL_Complex8*)(&m_acMatrix[ulStart]), (MKL_INT)DBF_LOG19_SIZE,
		0.0f,
		(MKL_Complex8*)m_acCovariance, (MKL_INT)NUM_CHANNELS );

	for (ULONG i = 0; i < NUM_CHANNELS; i++)
	{
		for (ULONG j = i + 1; j < NUM_CHANNELS; j++)
		{
			float re = m_acCovariance[i * NUM_CHANNELS + j].re;
			float im = m_acCovariance[i * NUM_CHANNELS + j].im;
			m_acCovariance[i * NUM_CHANNELS + j].im = -im;  // conj(C[i,j]) -> R[i,j]
			m_acCovariance[j * NUM_CHANNELS + i].re = re;  // R[j,i] = conj(R[i,j])
			m_acCovariance[j * NUM_CHANNELS + i].im = im;
		}
	}

	// Normalize the sample covariance matrix
	EMSCOMPLEX cScaleFactor;
	cScaleFactor.re = 1.0f / ((float)(ulNumSamples * (float)ulNumSamples * (float)ulNumSamples));
	cScaleFactor.im = 0.0f;
...
EMS_RESULT
CDigitalBeamFormer::ComputeCovariance_cblas_cherk( ULONG ulNumSamples )
{
	// Note: Remove 1st 50 kHz given elements are centred at 150 kHz.
	// ulStart is the 50 kHz offset given the sample rate 500000.
	ULONG ulStart = DBF_LOG19_SIZE / 5;
...
#pragma omp parallel for schedule(static)
	for (int icell = 0; icell < DBF_NUM_ELEMENTS; icell++)
	{
		float* pTemp1 = m_afChanTemp1[icell];
		float* pTemp2 = m_afChanTemp2[icell];
		EMSCOMPLEX* pcTemp1 = m_acChanTemp1[icell];
		EMSCOMPLEX* pcTemp2 = m_acChanTemp2[icell];

		// Convert integer to float; zero only the FFT zero-padding tail
		memset( &pTemp1[m_ulSamplesPerChannel], 0, (DBF_LOG20_SIZE - m_ulSamplesPerChannel) * sizeof( float ) );
		memset( &pTemp2[m_ulSamplesPerChannel], 0, (DBF_LOG20_SIZE - m_ulSamplesPerChannel) * sizeof( float ) );

		//Skip first channel, set index = icell + 1
		for (int jsample = 0, index = icell + 1; jsample < (int)m_ulSamplesPerChannel; jsample++, index += (int)m_ulChannels)
		{
			pTemp1[jsample] = (float)m_asRawTimeSeries[index] * fScaleFactorInverse;
		}

		double dLocalMean = 0.0, dLocalStdDev = 0.0;
		dLocalStdDev = _EMSsMeanStdDev( pTemp1, m_ulSamplesPerChannel, &dLocalMean );

		if (dLocalStdDev == 0.00)
			continue;

		m_aMaintenance.dMean[icell] = dLocalMean;
		m_aMaintenance.dStdDev[icell] = dLocalStdDev;

		_EMSsbNormalize( pTemp1, pTemp2, m_ulSamplesPerChannel, dLocalMean, dLocalStdDev );

		emssRealFftNip( pTemp2, pcTemp1, DBF_LOG20, EMS_SPL_FWD );

		// Assume 3*ulOffset50 < DBF_LOG19_SIZE
		memcpy( &pcTemp2[0], &pcTemp1[ulOffset50], (DBF_LOG19_SIZE - ulOffset50) * sizeof( EMSCOMPLEX ) );

		if (bTimeFreqFlag)
		{
			// Convert to complex time series
			emscFftNip( &pcTemp2[0], pcTemp1, DBF_LOG19, EMS_SPL_INV );
			// Store downconverted time series for each channel (Note: The sample rate remains unchanged)
			memcpy( &m_acMatrix[icell * DBF_LOG19_SIZE], pcTemp1, DBF_LOG19_SIZE * sizeof( EMSCOMPLEX ) );
		}
		else
		{
			// Store frequency domain data for each channel (Note: The sample rate remains unchanged)
			memcpy( &m_acMatrix[icell * DBF_LOG19_SIZE], pcTemp2, DBF_LOG19_SIZE * sizeof( EMSCOMPLEX ) );
		}
	}
